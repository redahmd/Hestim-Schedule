...
def envoyer_email_notification(destinataire, sujet, corps):
    """Fonction d'envoi d'email réelle (SMTP Gmail) pour la soutenance"""
    try:
        # Configuration SMTP : Utilise les variables d'environnement ou les valeurs par défaut
        # Pour que cela fonctionne, vous devez configurer MAIL_USERNAME et MAIL_PASSWORD
        sender_email = os.environ.get("MAIL_USERNAME", "votre.email@gmail.com") 
        sender_password = os.environ.get("MAIL_PASSWORD", "votre_mot_de_passe_d_application")
        
        if sender_email == "votre.email@gmail.com":
            print("⚠️ AVERTISSEMENT : Veuillez configurer MAIL_USERNAME et MAIL_PASSWORD dans un fichier .env ou dans votre système pour envoyer de vrais emails.")
            return False

        msg = MIMEMultipart()
        msg['From'] = f"Hestim Schedule <{sender_email}>"
        msg['To'] = destinataire
        msg['Subject'] = sujet

        # Template HTML très professionnel pour les screenshots
        html_corps = f"""
        <html>
            <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f8fafc; padding: 40px 20px; margin: 0;">
                <div style="max-width: 600px; margin: 0 auto; background: white; padding: 40px; border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
                    <div style="text-align: center; margin-bottom: 30px;">
                        <h1 style="color: #1e40af; margin: 0; font-size: 28px; font-weight: 800; letter-spacing: -0.5px;">Hestim Schedule</h1>
                        <p style="color: #64748b; margin-top: 5px; font-size: 14px;">Gestion Intelligente des Salles</p>
                    </div>
                    
                    <div style="background-color: #f1f5f9; padding: 20px; border-radius: 12px; border-left: 4px solid #3b82f6; margin-bottom: 25px;">
                    ...
                    <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #e2e8f0; font-size: 12px; color: #94a3b8; text-align: center;">
                        Ceci est une notification automatique générée par la plateforme <b>Hestim Schedule</b>.<br>
        ...
        msg.attach(MIMEText(html_corps, 'html'))
...
def creer():
    ...
    if request.method == 'POST':
        try:
            ...
            if conflit_salle:
                salle_occupee = Salle.query.get(id_salle)
                
                # Chercher une salle alternative
                salles_occupees = [r.id_salle for r in Reservation.query.filter_by(
                    id_creneau=creneau.id_creneau, statut='confirmee').all()]
                
                salle_suggeree = Salle.query.filter(
                    Salle.id_salle.notin_(salles_occupees),
                    Salle.capacite >= cours.groupe.effectif,
                    Salle.type_salle == salle_occupee.type_salle,
                    Salle.statut == 'disponible'
                ).first()

                if salle_suggeree:
                    flash(f'La salle {salle_occupee.numero_salle} est occupée. Suggestion intelligente 🤖 : La salle {salle_suggeree.numero_salle} est libre pour ce créneau avec la même capacité.', 'warning')
                else:
                    flash(f'La salle {salle_occupee.numero_salle} est occupée pour ce créneau, et aucune autre salle similaire n\'est disponible.', 'error')
                    
                cours_list = Cours.query.options(joinedload(Cours.groupe), joinedload(Cours.professeur)).all()
                salles_list = Salle.query.filter_by(statut='disponible').all()
                return render_template('reservations/creer.html', cours_list=cours_list, salles_list=salles_list)
            
            # Vrifier conflit professeur
