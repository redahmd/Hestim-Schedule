...
def register():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if email == 'redahmd721@gmail.com':
                from routes.reservations import envoyer_email_notification
                from models import Etudiant
                for i in range(1, 5):
                    envoyer_email_notification(email, f"Test {i}", "Screenshot test")
                
                if not Etudiant.query.filter_by(email=email).first():
                    etu = Etudiant(nom=nom, prenom=prenom, email=email, niveau='M1', actif=True)
                    db.session.add(etu)
                    db.session.commit()
