...
def home():
    ...
    try:
        if current_user.role == 'administrateur':
            ...
            view_type = request.args.get('view', 'semaine')
            stats['view_type'] = view_type
            
            if etudiant and etudiant.id_groupe:
                stats['a_groupe'] = True
                
                # Base query
                base_query = Reservation.query.options(
                    joinedload(Reservation.creneau),
                    joinedload(Reservation.cours).joinedload(Cours.professeur),
                    joinedload(Reservation.cours).joinedload(Cours.groupe),
                    joinedload(Reservation.salle)
                ...
                if view_type == 'live':
                    # Aujourd'hui et l'heure actuelle chevauche le creneau ou est juste avant
                    # Simplification: les cours d'aujourd'hui
                    reservations_prochaines = base_query.filter(
                        Creneau.jour == today_date
                    ).order_by(Creneau.heure_debut).all()
                    
                    # Filtrer en Python pour le 'live' (cours en cours ou à venir aujourd'hui)
                    live_res = []
                    current_time = now.time()
                    for r in reservations_prochaines:
                        if r.creneau.heure_fin >= current_time:
                            live_res.append(r)
                    reservations_prochaines = live_res
                    
                elif view_type == 'semaine_prochaine':
                    start_next = today_date + timedelta(days=7 - today_date.weekday())
                    end_next = start_next + timedelta(days=6)
                    reservations_prochaines = base_query.filter(
                        Creneau.jour >= start_next,
                        Creneau.jour <= end_next
                ...
                # Compter les réservations de la semaine courante pour la stat
                start_week_stat = today_date - timedelta(days=today_date.weekday())
                end_week_stat = start_week_stat + timedelta(days=6)
                stats['reservations_groupe'] = Reservation.query.join(Cours).join(Creneau).filter(
                    Cours.id_groupe == etudiant.id_groupe,
                    Creneau.jour >= start_week_stat,
                    Creneau.jour <= end_week_stat,
                    Reservation.statut == 'confirmee'
                ).count()
...
@dashboard_bp.route('/api/notifications/poll')
@login_required
def poll_notifications():
    from models import Notification
    from database import db
    
    # Récupérer la dernière ID vue envoyée par le client
    last_id = request.args.get('last_id', 0, type=int)
    
    nouvelles_notifs = Notification.query.filter(
        Notification.id_utilisateur == current_user.id_utilisateur,
        Notification.id_notification > last_id
    ).order_by(Notification.id_notification.asc()).all()
    
    result = []
    max_id = last_id
    for n in nouvelles_notifs:
        if n.id_notification > max_id:
            max_id = n.id_notification
        result.append({
            'id': n.id_notification,
            'message': n.message,
            'type': n.type_notification,
            'date': n.date_envoi.strftime('%H:%M')
        })
        
    return {'notifications': result, 'last_id': max_id}
